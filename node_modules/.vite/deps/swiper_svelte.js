...
function children2(selector) {
  const children3 = [];
  for (let i = 0; i < this.length; i += 1) {
    const childNodes = this[i].children;
    for (let j = 0; j < childNodes.length; j += 1) {
      if (!selector || $(childNodes[j]).is(selector)) {
        children3.push(childNodes[j]);
      }
    }
  }
  return $(children3);
...
function getChangedParams(swiperParams, oldParams, children3, oldChildren, getKey) {
  const keys = [];
  if (!oldParams)
    return keys;
  const addKey = (key) => {
  ...
  if (children3 && oldChildren) {
    const oldChildrenKeys = oldChildren.map(getKey);
    const childrenKeys = children3.map(getKey);
    if (oldChildrenKeys.join("") !== childrenKeys.join(""))
      addKey("children");
    if (oldChildren.length !== children3.length)
      addKey("children");
  }
...
function create_dynamic_element(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      svelte_element0_data = get_spread_update(svelte_element0_levels, [{ class: "swiper-wrapper" }]);
      if (/-/.test(
        /*wrapperTag*/
        ctx2[2]
      )) {
      ...
      svelte_element1_data = get_spread_update(svelte_element1_levels, [
